...
##             Dont install if version same
            ...
            TestFrame.trxmStatus = [False,False]
            if self.burnLoad(0,image1Pos,bpLoad) and self.checkBurnResult(0,1,bpLoad) and self.burnSre(0,Radio_App1,bpSre):
                for i in range(TestFrame.trxmPos+1):
                    time.sleep(30)
                    if (TestFrame.trxmPos < 2)==(TestFrame.trxmPos==i):
                        if not self.checkConnectionStatus(i+1):                
                            ...
                            trxm_res[i] = False
                        elif not self.burnLoad(i+1,image1Pos,trxmLoad) or not self.checkBurnResult(i+1,1,trxmLoad) or not self.burnSre(i+1,Radio_App1,trxmSre):
                            self.setFaultInfo('Fail to burn TRXM%d load to flash region 1\n' % i)
                            trxm_res[i] = False
                        else:
                            trxm_res[i] = True
